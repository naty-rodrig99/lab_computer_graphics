...
public:
    ...
    //Methods
public:
    const dvec3& getOrigin() const { return mOrigin; }
    const dvec3& getDirection() const { return mDirection; }
    /// Compute the point on a ray for a given scalar (origin + lamda * direction)
    dvec3 pointOnRay(double lambda) const
    {
        return mOrigin +
               dvec3(mDirection[0] * lambda, mDirection[1] * lambda, mDirection[2] * lambda);
    }
