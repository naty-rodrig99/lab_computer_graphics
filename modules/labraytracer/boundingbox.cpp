...
{
    mMin = dvec3(std::min(bb.min()[0], mMin[0]), std::min(bb.min()[1], mMin[1]),
                 std::min(bb.min()[2], mMin[2]));
    mMax = dvec3(std::max(bb.max()[0], mMax[0]), std::max(bb.max()[1], mMax[1]),
                 std::max(bb.max()[2], mMax[2]));
}
...
bool BoundingBox::anyIntersection(const Ray& ray, double /*maxLambda*/) const
{
    ...
    const dvec3& direction = ray.getDirection();
    const dvec3& origin = ray.getOrigin();
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            if (origin[i] < mMin[i] ||
                origin[i] > mMax[i])//is this right? we might not reject some trivial cases
                return false;
        }
    }
...
void BoundingBox::expandByPoint(const dvec3& p)
{
    for (int i = 0; i < 3; ++i)
    {
