...
const ProcessorInfo ColorMixing::getProcessorInfo() const { return processorInfo_; }
...
ColorMixing::ColorMixing()
    ...
    , propColorB("ColorB", "Color B", vec4(0), vec4(0), vec4(1), vec4(.1f),
                 InvalidationLevel::InvalidOutput, PropertySemantics::Color)
    , propColorC("ColorC", "Color C", vec4(0), vec4(0), vec4(1), vec4(.1f),
                 InvalidationLevel::InvalidOutput, PropertySemantics::Color)
{
...
void ColorMixing::Mix(const size2_t& Resolution, glm::u8vec3* pRaw)
{
    ...
    // Mix!
    const vec3 ColorAB = propMixingMode.get() == 0 ? AdditiveColorMixing(ColorA, ColorB)
                                                   : SubtractiveColorMixing(ColorA, ColorB);
    const vec3 ColorBC = propMixingMode.get() == 0 ? AdditiveColorMixing(ColorB, ColorC)
                                                   : SubtractiveColorMixing(ColorB, ColorC);
    const vec3 ColorAC = propMixingMode.get() == 0 ? AdditiveColorMixing(ColorA, ColorC)
                                                   : SubtractiveColorMixing(ColorA, ColorC);
    const vec3 ColorABC = propMixingMode.get() == 0 ? AdditiveColorMixing(ColorAB, ColorC)
                                                    : SubtractiveColorMixing(ColorAB, ColorC);
